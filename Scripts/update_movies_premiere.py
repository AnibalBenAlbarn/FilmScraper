...
NEW_MOVIES_URL = f"{BASE_URL}/peliculas-estreno"
...
def extract_movie_details(movie_url, worker_id=0, db_path=None):
    logger.info(f"[Worker {worker_id}] Extrayendo detalles de la película: {movie_url}")
...
def generate_update_report(start_time, processed_movies):
    ...
    # Generar informe
    report = f"""
INFORME DE ACTUALIZACIÓN DE PELÍCULAS DE ESTRENO - {end_time.strftime('%Y-%m-%d %H:%M:%S')}
===========================================================================
...
"""
    ...
    report += """
===========================================================================
Este es un mensaje automático generado por el sistema de actualizaciónde películas.
"""
...
# Función principal para procesar películas de estreno
def process_new_movies(max_pages=None, db_path=None):
    start_time = datetime.now()
    logger.info(f"Iniciando procesamiento de películas de estreno: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        ...
        logger.info(f"Procesamiento de películas de estreno completado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    process_new_movies(args.max_pages, args.db_path)